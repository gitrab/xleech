...
$HTMLOUT .= "<tr class='tableb'><td><b>Invite code</b></td><td><b>{$lang['invites_date']}</b></td><td><b>{$lang['invites_delete']}</b></td><td><b>{$lang['invites_status']}</b></td></tr>";

for ($i = 0; $i < $num_row; ++$i)
{
$fetch_assoc = mysql_fetch_assoc($select);                                  

$HTMLOUT .= "<tr class='tableb'><td>".$fetch_assoc['code']." <a href='?do=send_email&amp;id=".(int)$fetch_assoc['id']."'><img src='{$TBDEV['pic_base_url']}email.gif' border='0' alt='Email' title='Send Email' /></a></td><td>" . get_date($fetch_assoc['invite_added'], '', 0,1)."</td>";
...
$HTMLOUT .= "<tr class='tableb'><td colspan='7' align='center'><form action='?do=create_invite' method='post'><input type='submit' value='{$lang['invites_create']}' style='height: 20px' /></form></td></tr>";
$HTMLOUT .= "</table>"; 
